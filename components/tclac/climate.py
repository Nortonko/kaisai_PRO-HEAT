...
SUPPORTED_FAN_MODES_OPTIONS = {
    "AUTO": ClimateMode.CLIMATE_FAN_AUTO,  # Dostupné vždy
...
SUPPORTED_SWING_MODES_OPTIONS = {
    "OFF": ClimateSwingMode.CLIMATE_SWING_OFF,  # Dostupné vždy
...
SUPPORTED_CLIMATE_MODES_OPTIONS = {
    "OFF": ClimateMode.CLIMATE_MODE_OFF,  # Dostupné vždy
    "AUTO": ClimateMode.CLIMATE_MODE_HEAT_COOL,  # Dostupné vždy
...
SUPPORTED_CLIMATE_PRESETS_OPTIONS = {
    "NONE": ClimatePreset.CLIMATE_PRESET_NONE, # Dostupné vždy
...
# Kontrola konfigurácie rozhrania a prevzatie predvolených hodnôt
    ...
    if CONF_VISUAL in config:
        ...
        if CONF_MIN_TEMPERATURE in visual_config:
            ...
            if min_temp < TCLAC_MIN_TEMPERATURE:
                raise cv.Invalid(f"Zadaná minimálna teplota rozhrania {min_temp} je nižšia než povolená {TCLAC_MIN_TEMPERATURE} pre klimatizáciu")
        else:
        ...
        if CONF_MAX_TEMPERATURE in visual_config:
            ...
            if max_temp > TCLAC_MAX_TEMPERATURE:
                raise cv.Invalid(f"Zadaná maximálna teplota rozhrania {max_temp} je vyššia než povolená {TCLAC_MAX_TEMPERATURE} pre klimatizáciu")
        else:
        ...
        if CONF_TEMPERATURE_STEP in visual_config:
            ...
            if ((int)(temp_step * 2)) / 2 != temp_step:
                raise cv.Invalid(f"Zadaný krok teploty {temp_step} nie je správny, musí byť násobkom 1")
        else:
...
# Kontrola konfigurácie komponentu a prevzatie predvolených hodnôt
...
# Registrácia udalostí zapnutia a vypnutia displeja klimatizácie
...
# Registrácia udalostí zapnutia a vypnutia pípania klimatizácie
@automation.register_action(
...
# Registrácia udalostí zapnutia a vypnutia LED komunikácie modulu
@automation.register_action(
...
# Registrácia udalostí zapnutia a vypnutia vynúteného použitia nastavení
@automation.register_action(
...
# Registrácia udalosti nastavenia vertikálnej fixácie lamely
@automation.register_action(
...
# Registrácia udalosti nastavenia horizontálnej fixácie lamiel
@automation.register_action(
...
# Registrácia udalosti nastavenia vertikálneho kývania lamely
@automation.register_action(
...
# Registrácia udalosti nastavenia horizontálneho kývania lamiel
@automation.register_action(
