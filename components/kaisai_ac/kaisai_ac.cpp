...
// Názvy stupňov ventilátora podľa kódu poľa 0x05
...
void KaisaiAC::send_command_(const std::vector<uint8_t> &payload) {
  ...
  std::vector<uint8_t> f = {0xA5, 0x01,           0x01, 0x21, seq, 0x00,
                            0x00, total,          (uint8_t)(crc >> 8), (uint8_t)(crc & 0xFF),
                            0x0A, 0x0A};
...
void KaisaiAC::parse_buffer_() {
  while (this->rx_buf_.size() >= 8) {
    ...
    if (len < 12 || len > 200) {  // nezmyselná dĺžka -> resync
      this->rx_buf_.erase(this->rx_buf_.begin());
      continue;
    }
    if (this->rx_buf_.size() < len)
      break;  // ešte nemáme celý rámec

    // CRC cez hlavičku [0:8] + telo [10:len] (bez 2 CRC bajtov)
    ...
    if (crc == got) {
      // stavový rámec z klimatizácie má marker 0C 0C
      ...
      this->rx_buf_.erase(this->rx_buf_.begin());  // zlé CRC -> resync o 1 bajt
    }
  }
}
...
void KaisaiAC::process_status_(const uint8_t *d, size_t n) {
  ...
  // power (00 01 P 00 02) a mode (00 12 M 00 DF)
  ...
  // nastavená teplota (00 02 00 00 HI LO)
  ...
  // aktuálna teplota (00 03 00 00 HI LO)
  ...
  // ventilátor (00 05 F 00 0C | 72)
  if (ext8b(d, n, 0x05, 0x0C, 0x72, v) && v < 8 && v != this->fan_code_state_) {
    this->fan_code_state_ = v;
    this->custom_fan_mode = std::string(FAN_NAMES[v]);
    this->fan_mode.reset();
    changed = true;
  }

  // swing vertikálny (00 0C V 00 0D) a horizontálny (00 0D H 00 0E)
  bool sw_ch = false;
  ...
  // polohy lamiel: vertikálna (00 11 X 00 12), horizontálna (00 0E X 00 11)
  if (ext8(d, n, 0x11, 0x12, v) && v != this->vane_v_code_) {
    this->vane_v_code_ = v;
    if (this->sel_v_ != nullptr)
      for (auto &o : VANE_V)
        if (o.code == v) {
          this->sel_v_->publish_state(o.name);
          break;
        }
  }
  if (ext8(d, n, 0x0E, 0x11, v) && v != this->vane_h_code_) {
    this->vane_h_code_ = v;
    if (this->sel_h_ != nullptr)
      for (auto &o : VANE_H)
        if (o.code == v) {
          this->sel_h_->publish_state(o.name);
          break;
        }
  }

  // binárne funkcie (samostatné rámce)
...
void KaisaiAC::recompute_mode_() {
  ...
  switch (this->mode_field_) {
    ...
    default: this->mode = climate::CLIMATE_MODE_AUTO; break;  // 0
  }
}
...
const char *KaisaiAC::fan_name_(uint8_t code) { return code < 8 ? FAN_NAMES[code] : "AUTO"; }
int KaisaiAC::fan_code_(const std::string &name) {
  for (int i = 0; i < 8; i++)
    if (name == FAN_NAMES[i])
      return i;
  return -1;
...
climate::ClimateTraits KaisaiAC::traits() {
  ...
  t.set_supported_custom_fan_modes({"AUTO", "MUTE", "LOW", "LOW-MID", "MID", "MID-HIGH", "HIGH", "TURBO"});
...
void KaisaiAC::control(const climate::ClimateCall &call) {
  if (call.get_mode().has_value()) {
    ...
    if (m == climate::CLIMATE_MODE_OFF) {
      ...
      switch (m) {
        ...
        default: mf = 0; break;  // AUTO / HEAT_COOL
      }
  ...
  if (call.get_custom_fan_mode().has_value()) {
    int c = this->fan_code_(*call.get_custom_fan_mode());
    if (c >= 0) {
      this->send_command_({0x00, 0x05, (uint8_t) c});
      this->custom_fan_mode = *call.get_custom_fan_mode();
      this->fan_mode.reset();
      this->fan_code_state_ = (uint8_t) c;
    }
  }
...
void KaisaiAC::register_select(bool horizontal, KaisaiSelect *sel) {
  if (horizontal)
    this->sel_h_ = sel;
  else
    this->sel_v_ = sel;
...
void KaisaiAC::set_vane_by_name(bool horizontal, const std::string &name) {
  const VaneOpt *tab = horizontal ? VANE_H : VANE_V;
  size_t cnt = horizontal ? (sizeof(VANE_H) / sizeof(VaneOpt)) : (sizeof(VANE_V) / sizeof(VaneOpt));
  for (size_t i = 0; i < cnt; i++)
    if (name == tab[i].name) {
      uint8_t id = horizontal ? 0x0E : 0x11;
...
void KaisaiAC::setup() {
  ...
  this->last_poll_ = 0;  // pošli prvý poll hneď
}
...
void KaisaiAC::loop() {
  ...
  // odosielanie z fronty (rozostupene, aby sme nezahltili linku)
  ...
  // periodický poll (udržuje AC "v obraze")
...
void KaisaiSelect::setup() {
  if (this->parent_ != nullptr)
    this->parent_->register_select(this->horizontal_, this);
}
void KaisaiSelect::control(const std::string &value) {
  if (this->parent_ != nullptr)
    this->parent_->set_vane_by_name(this->horizontal_, value);
